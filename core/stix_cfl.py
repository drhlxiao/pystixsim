...
from core import stix_detector
...
class CFL(object):
    def __init__(self, x_arcsec=0, y_arcsec=0):
        self.det_idx = 8
        self.detector = stix_detector.StixDetector
        ...
        self.top_hole_vtx = None
        self.bottom_hole_vtx = None
    ...
    def project(self, x_arcsec=0, y_arcsec=0):
        ...
        area = {}
        sum_area = 0
        self.top_hole_vtx = [(p[0] + sgp.r_front_detector * math.tan(x_ang),
                              p[1] + sgp.r_front_detector * math.tan(y_ang))
                             for p in sgp.cfl_hole_coords['top']]
        #here it should be + because STIX is mounted on spacecraft
        self.bottom_hole_vtx = [(p[0] + sgp.r_front_detector * math.tan(x_ang),
                                 p[1] + sgp.r_front_detector * math.tan(y_ang))
                                for p in sgp.cfl_hole_coords['bottom']]
        self.bottom_shadow = Polygon(self.bottom_hole_vtx)
        self.top_shadow = Polygon(self.top_hole_vtx)
    ...
    def plot(self, fig=None, ax=None, ax2=None, draw_pattern=True, title=True):
        ...
        for name, path in sgp.cfl_hole_coords.items():
        ...
        for k, v in cfl_pro_paths.items():
            ...
            if k == 'outer':
                color = 'grey'
            stix_detector.ply2path(v, ax, fc=color, ec=color,alpha=1)
